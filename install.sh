...
# Unraid runs the dashboard as a container now (Community Applications), so this
# script no longer installs the flash-launcher + 'go' script setup it used to.
# ./uninstall.sh still removes that older install — see it for the migration.
if [ -f /etc/unraid-version ]; then
  cat <<'EOT'
Unraid detected — run the dashboard as a container instead of installing it here.

  Community Applications:  search "topographer" and hit Install
  or by hand:              docker run -d --name topographer --network host \
                             -v /mnt/user/appdata/topographer:/app/out \
                             ghcr.io/fugginold/topographer:latest

Appdata is the same directory the old native install used, so saved topologies
and widgets carry over untouched.

Already have the older native install? ./uninstall.sh removes it (flash launcher
+ the boot 'go' hook) and leaves your data in place.
EOT
  exit 1
fi
