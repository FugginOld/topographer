...
1. **Fetch + install (no git needed):**
   ...
   > **Unraid: don't use this one-liner — run the container instead** (next box).
   > `install.sh` used to persist the server with a flash launcher + a boot `go`
   > hook; that path is retired. On an Unraid host it now tells you to install the
   > container. Already on the old native install? `./uninstall.sh` removes it and
   > leaves `/mnt/user/appdata/topographer` alone, so the container picks your data
   > straight up.
